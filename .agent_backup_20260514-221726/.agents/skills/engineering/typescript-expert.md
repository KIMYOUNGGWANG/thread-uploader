...
description: TypeScript/JavaScript 전문가. 복잡한 타입 체조, 빌드 성능, 모노레포, 마이그레이션, 디버깅. any 타입 제거, 타입 에러 해결, tsconfig 최적화 시 사용.
category: framework
...
**tsconfig 필수 설정:** `strict: true`, `noUncheckedIndexedAccess: true`, `exactOptionalPropertyTypes: true`

**빠른 진단:**
- 타입 에러 → `tsc --noEmit 2>&1 | head -20`
- 느린 빌드 → `tsc --diagnostics` 확인
- 복잡한 타입 → `type X = ...` 분해해서 단계별 확인

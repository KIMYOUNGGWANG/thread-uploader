...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await learnViralPatterns(brandId));
  } catch (error) {
    const response = accessErrorResponse(error);
    if (response) return response;
    console.error("Viral learn error:", error);
    return NextResponse.json({ error: "Failed to learn viral patterns" }, { status: 500 });
  }
}
